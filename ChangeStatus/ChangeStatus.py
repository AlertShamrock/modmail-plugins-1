...
class ChangeStatus(commands.Cog):
    ...
    async def start_the_status(self):
        await self.bot.change_presence(activity=discord.Listening(name=f"{self.first}"))
        await asyncio.sleep(10)
        await self.bot.change_presence(activity=discord.Listening(name=f"{self.second}"))
        await asyncio.sleep(10)
